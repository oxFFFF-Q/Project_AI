...
#TODO 如果身边有wood还移动，则惩罚
    ...
    if action == 0:
        reward = check_avoid_flame(current_state, X, Y, reward, r_avoid)
        reward = check_corner_bomb(current_state, X, Y, reward, r_avoid, r_stay)
    ...
    if action == 5:
        ...
        if current_state["ammo"] != 0:
            ...
            reward += (2 * r_same_action_penalty)
...
def check_corner_bomb(current_state, X, Y, reward, r_avoid, r_stay):
    # action 0 来躲避左上bomb
    find_bomb = False
...
def check_and_away_from_bomb(current_state, X, Y, new_X, new_Y, reward, r_get_away_from_bomb, r_get_close_to_bomb,
    ...
    if X - 1 >= 0 and current_state["board"][X - 1][Y] == 3 and (abs((X - 1) - new_X) + abs(Y - new_Y)) > 1:
        reward += r_get_away_from_bomb
    if X - 2 >= 0 and current_state["board"][X - 2][Y] == 3 and (abs((X - 2) - new_X) + abs(Y - new_Y)) > 2:
        reward += r_get_away_from_bomb
    elif X - 2 >= 0 and current_state["board"][X - 2][Y] == 3 and (abs((X - 2) - new_X) + abs(Y - new_Y)) <= 2:
        reward += r_get_close_to_bomb
    if X - 3 >= 0 and current_state["board"][X - 3][Y] == 3 and (abs((X - 3) - new_X) + abs(Y - new_Y)) > 3:
        reward += r_get_away_from_bomb
    elif X - 3 >= 0 and current_state["board"][X - 3][Y] == 3 and (abs((X - 3) - new_X) + abs(Y - new_Y)) <= 3:
        reward += r_get_close_to_bomb
    # 下
    if X + 1 <= 10 and current_state["board"][X + 1][Y] == 3 and (abs((X + 1) - new_X) + abs(Y - new_Y)) > 1:
        reward += r_get_away_from_bomb
    if X + 2 <= 10 and current_state["board"][X + 2][Y] == 3 and (abs((X + 2) - new_X) + abs(Y - new_Y)) > 2:
        reward += r_get_away_from_bomb
    elif X + 2 <= 10 and current_state["board"][X + 2][Y] == 3 and (abs((X + 2) - new_X) + abs(Y - new_Y)) <= 2:
        reward += r_get_close_to_bomb
    if X + 3 <= 10 and current_state["board"][X + 3][Y] == 3 and (abs((X + 3) - new_X) + abs(Y - new_Y)) > 3:
        reward += r_get_away_from_bomb
    elif X + 3 <= 10 and current_state["board"][X + 3][Y] == 3 and (abs((X + 3) - new_X) + abs(Y - new_Y)) <= 3:
        reward += r_get_close_to_bomb

    # 左
    if Y - 1 >= 0 and current_state["board"][X][Y - 1] == 3 and (abs(X - new_X) + abs((Y - 1) - new_Y)) > 1:
        reward += r_get_away_from_bomb
    if Y - 2 >= 0 and current_state["board"][X][Y - 2] == 3 and (abs(X - new_X) + abs((Y - 2) - new_Y)) > 2:
        reward += r_get_away_from_bomb
    elif Y - 2 >= 0 and current_state["board"][X][Y - 2] == 3 and (abs(X - new_X) + abs((Y - 2) - new_Y)) <= 2:
        reward += r_get_close_to_bomb
    if Y - 3 >= 0 and current_state["board"][X][Y - 3] == 3 and (abs(X - new_X) + abs((Y - 3) - new_Y)) > 3:
        reward += r_get_away_from_bomb
    elif Y - 3 >= 0 and current_state["board"][X][Y - 3] == 3 and (abs(X - new_X) + abs((Y - 3) - new_Y)) <= 3:
        reward += r_get_close_to_bomb

    # 右
    if Y + 1 <= 10 and current_state["board"][X][Y + 1] == 3 and (abs(X - new_X) + abs((Y + 1) - new_Y)) > 1:
        reward += r_get_away_from_bomb
    if Y + 2 <= 10 and current_state["board"][X][Y + 2] == 3 and (abs(X - new_X) + abs((Y + 2) - new_Y)) > 2:
        reward += r_get_away_from_bomb
    elif Y + 2 <= 10 and current_state["board"][X][Y + 2] == 3 and (abs(X - new_X) + abs((Y + 2) - new_Y)) <= 2:
        reward += r_get_close_to_bomb
    if Y + 3 <= 10 and current_state["board"][X][Y + 3] == 3 and (abs(X - new_X) + abs((Y + 3) - new_Y)) > 3:
        reward += r_get_away_from_bomb
    elif Y + 3 <= 10 and current_state["board"][X][Y + 3] == 3 and (abs(X - new_X) + abs((Y + 3) - new_Y)) <= 3:
        reward += r_get_close_to_bomb
...
def check_avoid_flame(current_state, X, Y, reward, r_avoid):
    can_move = True
    grids = []
    if X - 1 >= 0:
        grids.append(current_state["board"][X - 1][Y])
    if X + 1 <= 10:
        grids.append(current_state["board"][X + 1][Y])
    if Y - 1 >= 0:
        grids.append(current_state["board"][X][Y - 1])
    if Y + 1 <= 10:
        grids.append(current_state["board"][X][Y - 1])
